...
namespace BscTokenSniper
{
    public class SniperService : IHostedService
    {
        ...
        private readonly CancellationTokenSource _processingCancellation = new(TimeSpan.FromSeconds(30));
        ...
        public SniperService(IOptions<SniperConfiguration> options, RugHandler rugChecker, TradeHandler tradeHandler)
        {
            ...
            _factoryContract = _bscWeb3.Eth.GetContract(File.ReadAllText("./Abis/PairCreated.json"),
        ...
        private void CreateTokenPair(FilterLog log, Action<EventLog<PairCreatedEvent>> onNext)
        {
            ...
            if (onNext != null) onNext.Invoke(pairCreated);
        }

        private async Task<EthLogsObservableSubscription> StartClient()
        {
            _client = new StreamingWebSocketClient(_sniperConfig.BscNode);
            var filter = _bscWeb3.Eth.GetEvent<PairCreatedEvent>(_sniperConfig.PancakeswapFactoryAddress)
                .CreateFilterInput();
            ...
            _disposables.Add(subscription.GetSubscriptionDataResponsesAsObservable().Subscribe(t =>
            {
                ...
                {
                    Log.Logger.Error(e, nameof(StartClient));
                }
            }));
        ...
        private async Task NewAddLiquidityEvent(TransactionVO l)
        {
            ...
            var pairAddress =
                await getPairFunction.CallAsync<string>(_sniperConfig.LiquidityPairAddress, addLiquidityEvent.Token);
        ...
        private void KeepAliveClient()
        {
            while (!_disposed)
            {
                ...
                var disposable = handler.GetResponseAsObservable().Subscribe(t =>
                {
                    ...
                    Log.Logger.Information("Current Block: {0}", t);
                    _ = _addLiquidityBlockProcessor.ExecuteAsync(_currentBlock,
                        CancellationToken.None,
                        _currentBlock).ConfigureAwait(false);
                });
                try
                {
                ...
                {
                    Log.Logger.Error(e, nameof(KeepAliveClient));
                    Log.Logger.Information("Error from websocket, restarting client.");
        ...
        private async Task PairCreated(PairCreatedEvent pair)
        {
            var otherPairAddress =
                pair.Token0.Equals(_sniperConfig.LiquidityPairAddress, StringComparison.InvariantCultureIgnoreCase)
                    ? pair.Token1
                    : pair.Token0;
            ...
            var rugCheckPassed = _sniperConfig.RugCheckEnabled && !addressWhitelisted
                ? await _rugChecker.CheckRugAsync(pair)
                : true;
            var otherTokenIdx =
                pair.Token0.Equals(_sniperConfig.LiquidityPairAddress, StringComparison.InvariantCultureIgnoreCase)
                    ? 1
            ...
            Log.Logger.Information("Discovered Token Pair {0} Rug check Result: {1} Contract address: {2}", symbol,
                rugCheckPassed, otherPairAddress);
            if (!rugCheckPassed)
            {
            ...
            if (!honeypotCheck)
            {
                if (!addressWhitelisted)
                    ...
                    Log.Logger.Information("Buying Token pair: {0} WHITELISTED ADDRESS: {1}", symbol,
                        addressWhitelisted);
                await _tradeHandler.Buy(otherPairAddress, otherTokenIdx, pair.Pair, _sniperConfig.AmountToSnipe);
                return;
            }

            Log.Logger.Information("Starting Honeypot check for {0} with amount {1}", symbol,
                _sniperConfig.HoneypotCheckAmount);
            var buySuccess = await _tradeHandler.Buy(otherPairAddress, otherTokenIdx, pair.Pair,
                _sniperConfig.HoneypotCheckAmount, true);
            if (!buySuccess)
            {
            ...
            try
            {
                sellSuccess = await _tradeHandler.Sell(otherPairAddress, ownedToken.Amount - 1, marketPrice,
