...
namespace BscTokenSniper.Handlers
{
    public class RugHandler
    {
        ...
        private readonly string GetSourceUrl =
            "https://api.bscscan.com/api?module=contract&action=getsourcecode&address={0}&apikey={1}";
        ...
        {
            if (!_sniperConfig.RugdocCheckEnabled) return true;
            try
            {
                ...
                var valid = responseObject["status"].Value<string>()
                    .Equals("OK", StringComparison.InvariantCultureIgnoreCase);
                Log.Logger.Information("Rugdoc check token {0} Status: {1} RugDoc Response: {2}", token, valid,
                    rugdocStr);
                return valid;
            }
            catch (Exception e)
            {
                Log.Error(nameof(RugdocCheck), e);
                return false;
            }
        }
        ...
        {
            var otherPairAddress =
                pairCreatedEvent.Token0.Equals(_sniperConfig.LiquidityPairAddress,
                    StringComparison.InvariantCultureIgnoreCase)
                    ? pairCreatedEvent.Token1
                    : pairCreatedEvent.Token0;
            return await _bscWeb3.Eth.GetContract(_erc20Abi, otherPairAddress).GetFunction("symbol")
                .CallAsync<string>();
        }

        public async Task<bool> CheckRugAsync(PairCreatedEvent pairCreatedEvent)
        {
            if (pairCreatedEvent.Token0 != _sniperConfig.LiquidityPairAddress &&
                pairCreatedEvent.Token1 != _sniperConfig.LiquidityPairAddress)
            {
                Log.Logger.Warning("Target liquidity pair found for pair: {0} - {1}. Not bought",
                    pairCreatedEvent.Token0, pairCreatedEvent.Token0);
                return false;
            }
            ...
            var otherPairAddress =
                pairCreatedEvent.Token0.Equals(_sniperConfig.LiquidityPairAddress,
                    ...
                    ? pairCreatedEvent.Token1
                    : pairCreatedEvent.Token0;
            var otherPairIdx = pairCreatedEvent.Token0.Equals(_sniperConfig.LiquidityPairAddress,
                StringComparison.InvariantCultureIgnoreCase)
                ? 1
                : 0;
            Task<bool>[] rugCheckerTasks =
            {
        ...
        private async Task<bool> CheckMinLiquidity(PairCreatedEvent pairCreatedEvent, string token, int otherPairIdx)
        {
            ...
            if (!result)
            {
                Log.Logger.Warning("Not enough liquidity added to token {0}. Not buying. Only {1} liquidity added",
                    token, amountStr);
                return result;
            }

            Log.Logger.Information("Min Liqudity check for {0} token passed. Liqudity amount: {1}", currentPair,
                amountStr);
            ...
            if (_sniperConfig.MinimumPercentageOfTokenInLiquidityPool > 0)
            {
                ...
                var deadWalletBalanceTasks = _sniperConfig.DeadWallets
                    .Select(t => balanceOfFunction.CallAsync<BigInteger>(t)).ToList();
                await Task.WhenAll(deadWalletBalanceTasks);
                var deadWalletBalance = new BigInteger(0);
                ...
                {
                    Log.Logger.Error("Token {0} contract is giving a invalid supply", token);
                    return false;
                }
                ...
                Log.Logger.Information(
                    "Token {0} Token Amount in Pool: {1} Total Supply: {2} Burned {3} Total Percentage in pool: {4}% Min Percentage Liquidity Check Status: {5}",
                    token, tokenAmountInPool, totalTokenAmount, deadWalletBalance.ToString(),
                    percentageInPool.ToDouble(), result);
            }
        ...
        {
            var result =
                await _httpClient.GetAsync(string.Format(GetSourceUrl, otherTokenAddress, _sniperConfig.BscScanApikey));
            ...
            {
                Log.Logger.Warning("Bsc contract is not verified for token {0}", otherTokenAddress);
                return false;
            }
            ...
            if (_sniperConfig.CheckRouterAddressInContract)
                if (!srcCode.Contains(_sniperConfig.PancakeswapRouterAddress) &&
                    !srcCode.Contains(_sniperConfig.V1PancakeswapRouterAddress))
                {
                    Log.Logger.Information("Pancake swap router is invalid for token {0}", otherTokenAddress);
                    return false;
                }

            var containsRugCheckStrings =
                _sniperConfig.ContractRugCheckStrings.FirstOrDefault(t => srcCode.Contains(t));
            if (!string.IsNullOrEmpty(containsRugCheckStrings))
            {
                Log.Logger.Warning("Failed rug check for token {0}, contains string: {1}", otherTokenAddress,
                    containsRugCheckStrings);
                return false;
            }
