...
namespace BscTokenSniper.Handlers
{
    public class TradeHandler : IDisposable
    {
        ...
        private readonly List<TokensOwned> _ownedTokenList = new();
        private bool _stopped;

        public TradeHandler(IOptions<SniperConfiguration> options, RugHandler rugChecker)
        {
            _sniperConfig = options.Value;
            _bscWeb3 = new Web3(url: _sniperConfig.BscHttpApi,
                account: new Account(_sniperConfig.WalletPrivateKey, new BigInteger(_sniperConfig.ChainId)));
            ...
            _pancakeContract = _bscWeb3.Eth.GetContract(File.ReadAllText("./Abis/Pancake.json"),
                _sniperConfig.PancakeswapRouterAddress);
            _rugChecker = rugChecker;
            Start();
        }

        private static BigInteger Max { get; } =
            ...
            bool honeypotCheck = false)
        {
            try
            {
                ...
                var buyReturnValue = await buyFunction.SendTransactionAsync(_sniperConfig.WalletAddress, gas, amount, 0,
                    new[] { _sniperConfig.LiquidityPairAddress, tokenAddress },
                    _sniperConfig.WalletAddress,
                    DateTime.UtcNow.Ticks + _sniperConfig.TransactionRevertTimeSeconds);
                var reciept =
                    await _bscWeb3.TransactionManager.TransactionReceiptService.PollForReceiptAsync(buyReturnValue,
        ...
        public async Task<bool> Approve(string tokenAddress)
        {
            ...
            {
                Log.Logger.Warning("Could not approve sell for {0}", tokenAddress);
            }
        ...
        public async Task<bool> Sell(string tokenAddress, BigInteger amount, BigInteger outAmount, double slippage)
        {
            ...
            {
                var sellFunction =
                    _pancakeContract.GetFunction<SwapExactTokensForETHSupportingFeeOnTransferTokensFunction>();

                var gas = new HexBigInteger(_sniperConfig.GasAmount);
                var transactionAmount = new BigInteger((decimal)amount).ToHexBigInteger();

                var txId = await sellFunction.SendTransactionAsync(
                    new SwapExactTokensForETHSupportingFeeOnTransferTokensFunction
                    {
                        AmountOutMin = slippage == 0
                            ? outAmount
                            : new Fraction(outAmount).Subtract(new Fraction(slippage / 100.0).Multiply(outAmount))
                                .ToBigInteger(),
                        AmountIn = amount,
                ...
                var reciept =
                    await _bscWeb3.TransactionManager.TransactionReceiptService.PollForReceiptAsync(txId,
                        new CancellationTokenSource(TimeSpan.FromMinutes(2)));
        ...
        {
            if (price.Reserve0 == 0 || price.Reserve1 == 0) return BigInteger.Zero;
            var pricePerLiquidityToken = ownedToken.TokenIdx == 1
                ? new Fraction(price.Reserve0).Divide(price.Reserve1)
                : new Fraction(price.Reserve1).Divide(price.Reserve0);
        ...
        public void Start()
        {
            new Thread(MonitorPrices).Start();
        }
        ...
        private void MonitorPrices()
        {
            while (!_stopped)
            {
                for (var i = _ownedTokenList.Count - 1; i >= 0; i--)
                {
                    var ownedToken = _ownedTokenList[i];
                    if (ownedToken.FailedSell || ownedToken.HoneypotCheck) continue;
                    var price = _rugChecker.GetReserves(ownedToken.PairAddress).Result;
                    if (price.Reserve0 == 0 || price.Reserve1 == 0) continue;

                    var currentPrice = GetMarketPrice(price, ownedToken, ownedToken.BnbAmount);
                    var profitPerc = new Fraction(currentPrice).Subtract(ownedToken.SinglePrice)
                        .Divide(ownedToken.SinglePrice).Multiply(100);
                    Log.Logger.Information("Token: {0} Price bought: {1} Current Price: {2} Current Profit: {3}%",
                        ownedToken.Address, (decimal)ownedToken.SinglePrice, (decimal)currentPrice,
                        (decimal)profitPerc);
                    ...
                    if (profitPerc > new Fraction(_sniperConfig.ProfitPercentageMargin))
                    {
                        try
                        {
                            ownedToken.FailedSell = !Sell(ownedToken.Address, ownedToken.Amount - 1,
                                    GetMarketPrice(ownedToken, ownedToken.Amount - 1).Result,
                                    _sniperConfig.SellSlippage)
                                .Result;
                        }
                        catch (Exception e)
                        {
